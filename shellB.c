...
//position head or current to the right command name 
struct node* get(char inBuff[],struct node **start){

    struct node *current=*start;

   //check if it exit in history and make current point to
...
/*
save file after each input command
*/
void    saveHistory(struct node *start,FILE *ofp,int displayHistory,int index){
    fprintf(ofp,"%d\n",displayHistory);
    fprintf(ofp,"%d\n",index);
    start=start->next;
    while(start!=NULL){
...
/*load file and store in linked list to be used
*/
void    loadHistory(struct node *start,FILE *ifp,int *displayHistory,int *index,int *size){
    int no;
    char inputBuff[MAXLINE];
    fscanf(ifp,"%d",displayHistory);
    fscanf(ifp,"%d",index);
    ...
    while(1){
        ...
        addCommand(no,inputBuff,start,size,*displayHistory);
    }
}
...
int main(void)
{
    ...
    //check if file exit else create new file 
    if (file== NULL){                                   
    }else {
        loadHistory(head,file,&displayHistory,&index,&historySize);
        fclose(file);
    }
    if(file==NULL) {
